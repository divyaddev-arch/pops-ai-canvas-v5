...
import { DefaultTopAppBar } from './DefaultTopAppBar';
...
export const UnifiedLayout: React.FC<UnifiedLayoutProps> = ({ 
  ...
  const [internalActiveDrawerItemId, setInternalActiveDrawerItemId] = React.useState('');
  ...
  const renderContent = () => {
    ...
    if (activeId === 'growth' && activeDrawerItemId === 'career') {
      return <CareerCoachView userName={userName} userEmail={userEmail} />;
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-[#5F6368] w-full py-20">
        <div className="text-center">
          ...
          <div style={{ height: '1000px' }} className="flex items-center justify-center text-[#C4C7C5] mt-10 border border-dashed border-[#C4C7C5]">
            Scroll Test Spacer (1000px)
          </div>
        </div>
      </div>
    );
