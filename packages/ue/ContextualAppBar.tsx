...
import { Menu } from '../gm3-react-components/Menu';

export interface ContextualAppBarAction {
  id: string;
...
interface ContextualAppBarProps {
  title: string;
  onBackClick?: () => void;
  actions?: ContextualAppBarAction[];
  userInitial?: string;
}

const defaultActions: ContextualAppBarAction[] = [
  {
    id: 'help',
    icon: (
...
export const ContextualAppBar: React.FC<ContextualAppBarProps> = ({
  title,
  onBackClick,
  actions = [],
  userInitial = 'FL'
  ...
  return (
    <div className="w-full h-16 bg-[#F8FAFD] flex items-center justify-between px-4 font-google-sans" id="cee-contextual-app-bar">
      {/* Left side: Back + Title */}
      <div className="flex items-center gap-3">
        <IconButton onClick={onBackClick} className="text-[#444746]" id="back-button">
          <Icon>arrow_back</Icon>
        </IconButton>
        <span className="text-xl font-medium text-[#1F1F1F]" id="app-bar-title">{title}</span>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {/* Avatar / Profile (Copied from DefaultTopAppBar) */}
        <div className="relative ml-1 flex-shrink-0" id="profile-cluster">
          <div 
            className="w-8 h-8 bg-[#0B57D0] rounded-full flex items-center justify-center text-white text-base font-medium cursor-pointer relative"
            onClick={(e: React.MouseEvent<HTMLElement>) => {
              setAnchorEl(e.currentTarget);
              ...
              <div className="flex flex-col items-center px-6 mb-2">
                <div className="flex items-center w-full mb-3">
                  ...
                  <div className="flex flex-col overflow-hidden">
                    <span className="text-sm font-medium text-[#1F1F1F] truncate">First Last</span>
                    <span className="text-xs text-[#444746] truncate">username@google.com</span>
                  </div>
                </div>
