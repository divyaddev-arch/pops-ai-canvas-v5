...
const externalTooltipHandler = (context: any) => {
  ...
  reversedDataPoints.forEach((dataPoint: any, i: number) => {
    ...
    if (Array.isArray(rawData) && rawData.length === 2) {
      const min = parseFloat(rawData[0]).toLocaleString('en-US', { style: 'currency', currency: 'USD' });
      const max = parseFloat(rawData[1]).toLocaleString('en-US', { style: 'currency', currency: 'USD' });
      formattedValue = `${min} - ${max}`;
    } else if (typeof rawData === 'number' || !isNaN(parseFloat(rawData))) {
...
export const BarChart: React.FC<BarChartProps> = ({ data, options, title, subtitle, className }) => {
    ...
    useEffect(() => {
        ...
        const processedData = {
            ...
            datasets: data.datasets.map((dataset: any) => {
                ...
                return {
                    ...dataset,
                    originalBackgroundColor: originalColor,
                    backgroundColor: (context: any) => {
                        if (!context.chart.getActiveElements?.().length) {
                            return originalColor;
                        }
                        return context.active ? originalColor : hexToRgba(originalColor, 0.9);
                    }
                };
            })
        };
                        ...
                        generateLabels: (chart: any) => {
                            ...
                            return defaultLabels.map((label: any) => {
                                ...
                                if (meta.hidden) {
                                    ...
                                    const originalColor = dataset.originalBackgroundColor || dataset.backgroundColor;
            ...
            scales: {
                ...
                y: {
                     grid: {
                        ...
                        drawBorder: false,
                        color: gridColor,
                        lineWidth: (context: any) => (context.tick.value === 0 ? 1 : 0),
                    },
                    ticks: {
                        color: textColor,
                    },
        ...
        if (chart) {
            const updateScaleOptions = (scale: any) => {
                if (!scale) return;
            ...
            updateScaleOptions(chart.options.scales.x);
            updateScaleOptions(chart.options.scales.y);
