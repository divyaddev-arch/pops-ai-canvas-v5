...
export const SideAgent: React.FC<SideAgentProps> = ({
  onClose,
  onExpand,
  onMore,
  userName = 'Andie',
  ...
  React.useEffect(() => {
    ...
    chatRef.current = ai.chats.create({
      ...
      config: {
        systemInstruction: systemInstructions || "You are a helpful assistant.",
      },
    });
  }, [systemInstructions]);
  ...
  const handleSend = async (text: string) => {
    ...
    // Add user message
    const userMsg: Message = { type: 'user', text: trimmedText };
    ...
    try {
      ...
      const response = await chatRef.current.sendMessage({ message: trimmedText });
      ...
      <div className="w-full h-[72px] flex items-center justify-between px-4 py-4 border-b border-[#E8EAED]" id="side-agent-header">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <span className="text-base font-medium text-[#1F1F1F]">Supportive Mentor</span>
          </div>
        </div>
          ...
          <div className="flex flex-col items-center px-6 py-10 w-full">
            <div className="text-center">
              ...
              <p className="text-2xl font-normal leading-[32px] text-[#444746]">
                I'm here to support your growth. How are you feeling today?
              </p>
            </div>
            ...
            <div className="mt-8 flex flex-col gap-2 w-full max-w-[328px]">
              <button 
                onClick={() => handleSend("How can you help me grow?")}
              ...
              <button 
                onClick={() => handleSend("Prepare for a 1:1")}
                className="bg-[#E9EEF6] text-[#1F1F1F] rounded-full px-4 py-2.5 text-sm font-medium hover:bg-[#DFE4EC] transition-colors text-left truncate"
              >
                Prepare for a 1:1
              </button>
      ...
      <div className="w-full bg-white border-t border-[#E8EAED] p-4 flex flex-col gap-3 relative" id="side-agent-input-container">
        <div className="relative">
          <input 
            ...
            className="w-full h-14 border border-[#747775] rounded-full pl-6 pr-14 text-base text-[#1F1F1F] placeholder-[#444746] focus:outline-none focus:border-[#0B57D0]"
          />
          <button 
            onClick={() => handleSend(input)}
            className="absolute right-2 top-2 w-10 h-10 bg-transparent flex items-center justify-center rounded-full hover:bg-black/5 text-[#1F1F1F]"
            title="Send"
          >
            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-6 h-6">
        ...
        <p className="text-xs text-[#444746] text-center px-2">
          Your mentor is here to help, but always trust your intuition.
        </p>
      </div>
    </div>
  );
